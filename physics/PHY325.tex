...
% SKIPPED SOME STUFF: PEOPLE NOT HERE


\subsection{Diffusion Equation}\bigbreak

\[\pdv{}{t} = \pdv{}{x}(v_d \pdv{u}{x})\]
...
\[u(x, t) = 2 \sum_{n=1}^{\infty} \sin(\frac{\pi n}{2}) \sin(\pi n x) \exp(-\pi^2 n^2 v_d t)\]
...
\[\pdv{u}{x}|_j = \frac{u_{j+1}^n - u_j^n}{\Delta x}\]

The backwards derivative here is 
 
\[\pdv{u}{x}|_j = \frac{u_{j}^n - u_{j-1}^n}{\Delta x}\]

The centered derivative here is 

\[\pdv{u}{x}|_j = \frac{u_{j+1}^n - u_{j-1}^n}{2\Delta x}\]

The second derivative can also be found similarly in the same grid space.\\

The equation we will use then is

\[u_j^{n+1} = u^n_j + \frac{v_d \Delta t}{\Delta x^2}(u_{j+1}^n + u_{j-1}^n - 2u^n_{j})\]
...
\begin{equation} \label{eq:speedspund1}
	\dv{\psi}{t} = -c_s\pdv{\phi}{x}
\end{equation}

\begin{equation} \label{eq:speedspund2}
	\dv{\phi}{t} = -c_s\pdv{\psi}{x}
\end{equation}

Take now the partial derivative with respect to $x$ of equation \ref{eq:speedspund1} and the partial derivative with respect to $t$ of \ref{eq:speedspund2}. Then, we obtain the wave equation

\[\pdv[2]{\phi}{t} = c_s^2\pdv[2]{\phi}{x}\]

When implementing, we want to look at the quantity

\[S = \frac{c_s \Delta t}{\Delta}\]
